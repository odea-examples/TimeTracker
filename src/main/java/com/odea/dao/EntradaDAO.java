...
public class EntradaDAO extends AbstractDAO {
	...
	public void agregarEntrada(Entrada entrada){
		jdbcTemplate.update("INSERT INTO entrada VALUES (?,?,?,?,?,?,?)", 
				new Object [] {entrada.getId_entrada(), entrada.getId_proyecto(), entrada.getId_actividad(), entrada.getDuracion(), entrada.getNota(), entrada.getId_ticket(), entrada.getId_usuario()});
	}
	
	public Collection<Entrada> obtenerTodasLasEntradas(){
		Collection<Entrada> entradas = new ArrayList<Entrada>();
		
		entradas = jdbcTemplate.query("SELECT * FROM entrada", new RowMapper<Entrada>(){
			@Override
